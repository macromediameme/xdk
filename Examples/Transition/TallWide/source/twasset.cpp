...
STDMETHODIMP  		MoaCreate_CTallWideAsset(CTallWideAsset FAR * This)
{
	X_ENTER
...
STDMETHODIMP_(void) MoaDestroy_CTallWideAsset(CTallWideAsset FAR * This)
{
	X_ENTER
...
CTallWideAsset_IMoaMmXAsset::CTallWideAsset_IMoaMmXAsset(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
CTallWideAsset_IMoaMmXAsset::~CTallWideAsset_IMoaMmXAsset() {}

#ifdef USING_INIT_FROM_DICT
/* ------------------------------------ CTallWideAsset_IMoaInitFromDict Create/Destroy */
CTallWideAsset_IMoaInitFromDict::CTallWideAsset_IMoaInitFromDict(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
...
CTallWideAsset_IMoaNotificationClient::CTallWideAsset_IMoaNotificationClient(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::GetInMemMediaSize(MoaUlong FAR * pSizeBytes)
{
	X_ENTER
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::GetStreamOutMediaSize(MoaUlong FAR * pSizeBytes)
{
	X_ENTER
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::StreamOutProps(PIMoaStream pOutStream)
{
	...
	MoaError		err = kMoaErr_NoErr, utilsErr = kMoaErr_NoErr;
	PIMoaCallback 	pCallback 	= NULL;
	...
	err = pOutStream->Write(&(pObj->sharedAssetData.initialWidth), sizeof(pObj->sharedAssetData.initialWidth), NULL);
	pIMoaMmUtils->PrintMessage1("Init Width = %ld\n", pObj->sharedAssetData.initialWidth);
	err = pOutStream->Write(&(pObj->sharedAssetData.initialHeight), sizeof(pObj->sharedAssetData.initialHeight), NULL);
	pIMoaMmUtils->PrintMessage1("Init Height = %ld\n", pObj->sharedAssetData.initialHeight);
	err = pOutStream->Write(&(pObj->sharedAssetData.horizontalSpeed), sizeof(pObj->sharedAssetData.horizontalSpeed), NULL);
	pIMoaMmUtils->PrintMessage1("Horizontal Speed = %ld\n", pObj->sharedAssetData.horizontalSpeed);
	err = pOutStream->Write(&(pObj->sharedAssetData.verticalSpeed), sizeof(pObj->sharedAssetData.verticalSpeed), NULL);
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::StreamInProps(PIMoaStream pInStream)
{
	...
	if(getEnd == PROPERTY_BYTES) {
		...
		err = pInStream->Read(&(pObj->sharedAssetData.initialWidth), sizeof(pObj->sharedAssetData.initialWidth), NULL);
		pIMoaMmUtils->PrintMessage1("Init Width = %ld\n", pObj->sharedAssetData.initialWidth);
		err = pInStream->Read(&(pObj->sharedAssetData.initialHeight), sizeof(pObj->sharedAssetData.initialHeight), NULL);
		pIMoaMmUtils->PrintMessage1("Init Height = %ld\n", pObj->sharedAssetData.initialHeight);
		err = pInStream->Read(&(pObj->sharedAssetData.horizontalSpeed), sizeof(pObj->sharedAssetData.horizontalSpeed), NULL);
		pIMoaMmUtils->PrintMessage1("Horizontal Speed = %ld\n", pObj->sharedAssetData.horizontalSpeed);
		err = pInStream->Read(&(pObj->sharedAssetData.verticalSpeed), sizeof(pObj->sharedAssetData.verticalSpeed), NULL);
		pIMoaMmUtils->PrintMessage1("Vertical Speed = %ld\n", pObj->sharedAssetData.verticalSpeed);
	}
#endif /*MAC*/
	...
	if(getEnd == PROPERTY_BYTES) {
		...
		/*  Here we get the last saved values for our custom properties streamed in from Director*/
		pObj->sharedAssetData.initialWidth = MemXLong(tempinitialWidth);
		pIMoaMmUtils->PrintMessage1("Init Width = %ld\n", pObj->sharedAssetData.initialWidth);
		pObj->sharedAssetData.initialHeight = MemXLong(tempinitialHeight);
		pIMoaMmUtils->PrintMessage1("Init Height = %ld\n", pObj->sharedAssetData.initialHeight);
		pObj->sharedAssetData.horizontalSpeed = MemXLong(temphorizontalSpeed);
		pIMoaMmUtils->PrintMessage1("Horizontal Speed = %ld\n", pObj->sharedAssetData.horizontalSpeed);
		pObj->sharedAssetData.verticalSpeed = MemXLong(tempverticalSpeed);
		pIMoaMmUtils->PrintMessage1("Vertical Speed = %ld\n", pObj->sharedAssetData.verticalSpeed);
	}
#endif /*WIN*/
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::GetInMemPropsSize(MoaUlong FAR * pSizeBytes)
{
	X_ENTER
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::GetStreamOutPropsSize(MoaUlong FAR * pSizeBytes)
{
	X_ENTER
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::NewXActor(PMoaVoid FAR * ppXActor) 
{
	X_ENTER
 ...
 BOOL WIN_CALLBACK propertiesDlgProc(HWND hDlg, UINT iMsg, WPARAM wParm, LPARAM lParm)
{
	...
	MoaError 	err = kMoaErr_NoErr;
	static 		CTallWideAsset_IMoaMmXAsset FAR * This;

	switch (iMsg)
	{
		case WM_INITDIALOG:		/* initialize dialog */

			This = (CTallWideAsset_IMoaMmXAsset FAR *)lParm;
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::InvokePropsEditor()
{
	...
	MoaError			err = kMoaErr_NoErr, getErr = kMoaErr_NoErr;
	...
	DialogPtr		myDialog;
	RGBColor		saveColor;
	short			itemHit, True = 1, False = 0;
	MoaLong			setPropInt = TRUE, getPropInt = TRUE;
	short			iType;
	Handle	 		iHandle;
	Rect 			iRect;
	Str255			tempString255;
	StringPtr		tempStringPtr255;
	...
	if(myDialog)
	{
		...
		tempStringPtr255 = c2pstr(tempString);
		SetDialogItemText(iHandle, tempStringPtr255);
		/* initial height */
		...
		do 
		{
			...
			switch (itemHit) 
			{
				...
				case OK:	
					...
					p2cstr(tempString255);
					sscanf((char *) tempString255,"%ld",&pObj->sharedAssetData.initialWidth);

					/* initial height */
					GetDialogItem(myDialog, (short)INITIAL_HEIGHT, &iType, &iHandle, &iRect);
					GetDialogItemText( iHandle, tempString255);	
					p2cstr(tempString255);
					sscanf((char *) tempString255,"%ld",&pObj->sharedAssetData.initialHeight);

					/* vertical speed */
					GetDialogItem(myDialog, (short)VERTICAL_SPEED, &iType, &iHandle, &iRect);
					GetDialogItemText( iHandle, tempString255);	
					p2cstr(tempString255);
					sscanf((char *) tempString255,"%ld",&pObj->sharedAssetData.verticalSpeed);

					/* horizontal speed */
					GetDialogItem(myDialog, (short)HORIZONTAL_SPEED, &iType, &iHandle, &iRect);
					GetDialogItemText( iHandle, tempString255);	
					p2cstr(tempString255);
					sscanf((char *) tempString255,"%ld",&pObj->sharedAssetData.horizontalSpeed);
...
STDMETHODIMP CTallWideAsset_IMoaMmXAsset::ImageStaticThumb(PIMoaMmGC pGC, ConstPMoaRect pImageRect)
{
    ...
    if (err == kMoaErr_NoErr) pCallback = pObj->pCallback;
