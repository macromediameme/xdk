...
MoaError CDrAccessScript_IMoaMmXScript::XPly_ResolveFileName(PMoaDrCallInfo callPtr)
{
	...
	pObj->pMmValue->ValueToString(&value, fileName, PROPBUF_SIZE);
	
	/* Resolve it */
	pObj->pDrPlayer->ResolveFileName(fileName, fullPath, PROPBUF_SIZE );
	
	/* Convert back to value */
	pObj->pMmValue->StringToValue(fullPath,	&callPtr->resultValue);
...
MoaError CDrAccessScript_IMoaMmXScript::XCmm_ReadMedia(PMoaDrCallInfo callPtr)
{
	...
	pObj->pMmValue->StringToSymbol("SHAPE", &typeSymbol);
		...
		if ( (err = pObj->pMmValue->ValueToString( &gTempArg, filename, PROPBUF_SIZE)) == kMoaErr_NoErr) 
		{
			...
			if ( f ) 
			{
				...
				if ( hBlob ) 
				{
					...
					fseek(f, 0L, SEEK_SET);
					fread(pBlob, (size_t)siz, (size_t)1, f);
#ifdef WINDOWS
					/* Byte-swap here due to D5 bug. */
					DrFixMediaHeader((const char *)pBlob);
#endif
	...
	/* Release the dest cast mem */
	if ( pObj->pDrCastMem ) pObj->pDrCastMem->Release();
	pObj->pDrCastMem = NULL;

	/* Release the cast */
	if ( pObj->pDrCast ) pObj->pDrCast->Release();
	pObj->pDrCast = NULL;
...
MoaError CDrAccessScript_IMoaMmXScript::XScs_GetScoreSpriteProp(PMoaDrCallInfo callPtr)
{
	...
	pObj->pMmValue->StringToSymbol("frame", &symbol);
	err = pObj->pDrMovie->GetProp(symbol, &moaValue);
	...
	if ((err = pObj->pDrScore->SetCurFrameIndex(theFrame)) == kMoaErr_NoErr) 
	{
		...
		if ((err = pObj->pDrScore->GetSprite(iChanIndex, &pObj->pDrSprite)) == kMoaErr_NoErr) 
		{
			...
			err = pObj->pDrSprite->GetProp(symbol, &moaValue);
...
MoaError CDrAccessScript_IMoaMmXScript::XScs_SetScoreSpriteProp(PMoaDrCallInfo callPtr)
{
	...
	if ((err = pObj->pDrScore->SetCurFrameIndex(theFrame)) == kMoaErr_NoErr) 
	{
		...
		if ((err = pObj->pDrScore->GetSprite(iChanIndex, &pObj->pDrSprite)) == kMoaErr_NoErr) 
		{
			...
			err = pObj->pDrSprite->SetProp(symbol, &moaValue);
...
STDMETHODIMP MoaCreate_CDrAccessScript(CDrAccessScript FAR * This)
{
...
STDMETHODIMP_(void) MoaDestroy_CDrAccessScript(CDrAccessScript FAR * This)
{
...
CDrAccessScript_IMoaMmXScript::CDrAccessScript_IMoaMmXScript(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
CDrAccessScript_IMoaMmXScript::~CDrAccessScript_IMoaMmXScript() {}

#ifdef USING_INIT_FROM_DICT
/* -------------------------------- CDrAccessScript_IMoaInitFromDict Create/Destroy */
CDrAccessScript_IMoaInitFromDict::CDrAccessScript_IMoaInitFromDict(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
...
CDrAccessScript_IMoaNotificationClient::CDrAccessScript_IMoaNotificationClient(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
