...
STDMETHODIMP MoaCreate_CWackyDlgRegister( CWackyDlgRegister FAR * This )
{
...
STDMETHODIMP_(void) MoaDestroy_CWackyDlgRegister( CWackyDlgRegister FAR * )
{
	// see my note above.
	XSupport::Shutdown();
}
...
STDMETHODIMP CWackyDlgRegister_IMoaRegister::RegisterIcon(PIMoaDict pDict, ConstPMoaChar pResName, MoaBoolParam bColor)
{
	...
	pascalStringName[0] = ::strlen( pResName );
	
	if( pascalStringName[0] > 255 )
		pascalStringName[0] = 255;
	...
	LPVOID	pBitmap;
	DWORD	size;
	HRSRC 	hRsrc = ::FindResource( (HINSTANCE)openThemResources.GetFileRef(), pResName, MAKEINTRESOURCE(RT_BITMAP) );
	if( hRsrc )
	{
...
STDMETHODIMP MoaCreate_CWackyDlgAsset( CWackyDlgAsset FAR * This )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP_(void) MoaDestroy_CWackyDlgAsset( CWackyDlgAsset FAR * This )
{
	XMOAEXCEPT_ENTER
...
MoaLong CWackyDlgAsset::NativeTextWidth( PIMoaMmGC pGC, PMoaChar outString, ConstPMoaChar fontName, MoaShort size )
{
	...
	if( fontName && fontName[0] )
	{
		...
		mmFontName[0] = ::strlen( fontName );
		mmFontName[0] = mmFontName[0] > 255 ? 255 : mmFontName[0];
	...
	if( !fontName )
		...
		newFace = ::CreateFont(	-size, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 
	...
	if( newFace )
	{
		...
		::GetTextExtentPoint32( hDC, outString, ::strlen( outString ), &textDimensions );
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::GetProp( MoaMmSymbol /*property*/, PMoaMmValue /*pValue*/ )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::GetInMemMediaSize( MoaUlong FAR * pSizeBytes )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::GetStreamOutMediaSize( MoaUlong FAR * pSizeBytes )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::GetInMemPropsSize( MoaUlong FAR * pSizeBytes )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::GetStreamOutPropsSize( MoaUlong FAR * pSizeBytes )
{
	XMOAEXCEPT_ENTER
...
STDMETHODIMP CWackyDlgAsset_IMoaMmXAsset::ImageStaticThumb( PIMoaMmGC pGC, ConstPMoaRect pImageRect )
{
	...
	SetPortAndRestore port( (CGrafPtr)moaGCInfo.data.mac_window.windowPtr );
