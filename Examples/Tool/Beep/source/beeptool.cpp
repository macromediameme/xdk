...
/* ----------------------------------------------------------- MoaCreate_CTool */
STDMETHODIMP MoaCreate_CBeepTool(CBeepTool FAR * This)
{
	UNUSED(This);
	X_ENTER
...
/* ---------------------------------------------------------- MoaDestroy_CTool */
STDMETHODIMP_(void) MoaDestroy_CBeepTool(CBeepTool FAR * This)
{
	UNUSED(This);
	X_ENTER
...
CBeepTool_IMoaMmXTool::CBeepTool_IMoaMmXTool(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
CBeepTool_IMoaMmXTool::~CBeepTool_IMoaMmXTool() {}

#ifdef USING_INIT_FROM_DICT
/* ------------------------------------ CBeepTool_IMoaInitFromDict Create/Destroy */
CBeepTool_IMoaInitFromDict::CBeepTool_IMoaInitFromDict(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
...
CBeepTool_IMoaNotificationClient::CBeepTool_IMoaNotificationClient(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
...
STDMETHODIMP CBeepTool_IMoaNotificationClient::Notify(ConstPMoaNotifyID	nid, PMoaVoid pNData, PMoaVoid pRefCon)
{
	...
	MoaError	err = kMoaErr_NoErr;
		...
		if (!MoaEqualID(nid, &IID(IMuiDialog))) X_STD_RETURN(kMoaErr_BadInterface);
		
		PTMuiItem					recordArray = (PTMuiItem)pRefCon;
		if (!recordArray) X_STD_RETURN(kMoaErr_BadParam);
		...
		pObj->pCallback->QueryInterface(&IID_IMoaMmValue, (PPMoaVoid)&pMmValue);
		if (!pMmValue) X_RETURN(MoaError, kMoaErr_BadInterface);
				
		if (item->event == kMuiDialogItemClicked) switch (item->itemHit) {
