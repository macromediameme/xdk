...
/*
    ...
    991118  Added MOA_LIB_LINKED #define flag, for use by disk xtras which
			link moalib (or link another dll which exports moalib publics).
	
	971113	Added protection against being precompiled. (ags)
	
	961204	Release 1.6:
			
			Removed IMoaClassFactoryCallback.
			
			(srj)
			
	961204	Release 1.51:
			
			One more tweak to the tweaks for mixing C and C++ implementations.
			
			(srj)

	961204	Release 1.50:
			
			Made some tweaks to better allow for mixing C and C++ implementations.
			
			Also corrected implementation of X_RETURN.
			
			(srj)
			
	961022	Release 1.49:
			
			the a4saver class for 68k Macs under C++ was faulty; a4 would never get set
			up correctly if:
			-- you compiled for 68k Mac
			-- you compiled under C++
			-- you used X_ENTER (or macros that use it, like moa_try)

			this is now fixed. (srj)

	960627	Release 1.47:

			if IGNORE_STD_GUID is defined, we make DEFINE_STD_GUID the same as DEFINE_GUID.

			removed the "#define IClassFactory IMoaClassFactory" section to improve
			compatibility with MS headers. (srj)

			simplified all the two-step macros for MPW C (which couldn't handle that
			much macro space, alas).
			
			remove all usage of IN_X_CODE in favor of !MOA_APP_CODE.
			
			generally restructured the macros and symbols for A4-handling for 68k Macs.
			
			changed the type of "magicCookie" from PMoaVoid to a custom type.

			(srj)

	960606	Release 1.46:

			removed the "#define IUnknown IMoaUnknown" section to improve
			compatibility with MS headers. (srj)

	960220	Release 1.44:
			
			Major surgery. All platform-dependent stuff was moved out
			of this file into the edge file moatyxxx.h.

			(srj)
	
	960207	Release 1.43:
			
			Modified all the macros to use two-step macros for token pasting;
			this allows preprocessor definitions to be used for all
			class/interface names.
			
			Modified the definitions of DEFINE_GUID and VTABLE generating
			macros to place the data into a specific far data segment under Win16;
			this makes it easy to identify all the constant data generated
			by MOA and specify it in a .def file. NOTE: You must compile with the
			SMALLER_VTABLE switch to get vtables placed here. The MOA library files
			do this by default, but app code doesn't.

			Rearranged some macros to allow for easier mixing of C++ and C
			classes within the same Xtra. NOTE: if you do this, the BEGIN_XTRA
			block needs to be in a C file (not C++).
			
			(srj)
			
	960118	Release 1.42:
			
			Macintosh BEGIN_INTERFACE macro modified for Metrowerks 8 or later; they
			now provide the null entry at the start, so we don't need to.

			Don't use pragma options align for THINK C.

			(srj)

	951220	Release 1.41:
			
			Minor mods to compile with some early Windows headers:
			-- cast all MoaError results to MoaError
			-- #undef IID_NULL to avoid conflict with ours

	951128	Release 1.39:
			
			changed internal macros to be compatible with PPCC and some variations
			of MSVC2.x.
	...
	951026	Release 1.37:
			...
			changed MessageBox prototypes to "const char *". (srj)
	
	951003	Release 1.34:
			
			Include stdio.h and stdarg.h earlier.
			
			Removed gXtraFSSpec (mac only); if we load the Code Fragment using
			GetMemFragment then there isn't a convenient way to get it, and
			besides, it's not very useful anyway.
			
			Added #pragma export for the necessary exported Mac CFM functions
			(allowing you to skip the .exp file in Mwerks if you specify "use pragma").
			
			moved all error codes to moatypes.h.
			
			declare all vtables FAR to improve memory usage under Win16.
			"unsafe" vtables are also declared const.
			
			revise defs so that defining SMALLER_VTABLE before including
			this file allows choice of the smaller (but less safe) vtable declaration.
			
			(srj)
	
	950918	Release 1.32:
	
			Switch back to the old technique of BEGIN_VTABLE.
	
			A4_GLOBALS usage of SaveA4() needed a semicolon.
			
			WIN32 declaration of wvsprintfA needed some extra Windows stuff
			to avoid warnings.

			no longer use the symbol WIN16; now	infer Win 3.x vs. Win32 solely
			based on the presence or absence of WIN32 being defined.			

			change all slash-slash comments to slash-star comments.

			(srj)
	
	950901	Release 1.3.1:

			Revised the BEGIN_VTABLE macros to allow for more
			efficient static initialization.
			
			Assume IN_X_CODE unless MOA_APP_CODE is defined.
			
			Try to infer MACINTOSH from compiler flags.
			
			Leave INTERFACE undefined at end of file.
			
			explicitly define _debugstr, etc. as FAR _MOA_CDECL.
			make all args FAR as appropriate. use wvsprintf
			under Windows.
			
			The STD_CREATE_DESTROY macros should have declared
			This as a FAR pointer, but did not.
			
			changed InitBlock to CFragInitBlock.
			
			(srj)

	950809	Release 1.3:
			
			Many changes to allow for new caching scheme.
			
			(srj)
	
	950724	Release 1.2.1:
			
			removed warning from the STD_xxx_CREATE_DESTROY macros.
			
			added MoaFloat, MoaDouble, PMoaVoid, ConstPMoaVoid,
			MoaWide, and macros for MoaWide.
	...
	950612	Release 1.2:
			...
			Split from moaez.h to form moaxtra.h.
			
			Provide final C++ support.
			
			(srj)
	
	950523	Release 1.1.7:
			
			revised to allow use of this file without including <windows.h>
			under Windows (for Altura users). Define _NO_MOA_WINDOWS_INCLUDE_
			before including this file to prevent windows.h from being
			included.
			
			Require either WIN16 or WIN32 to be defined now (don't assume one).
			
			(srj)
	
	950515	Release 1.1.6:
			
			our function pointers all needed to be explicitly typed as to
			call type (some compiler setups may assume Pascal if not explicitly specified).
			If you need to declare your own callback functions for your interface, you
			should use the STDPROCPTR() macro (see the definition of MoaCreatedProc for
			an example).

			Removed the error for compiling C++ in Mwerks 68k because CW6 is now compatible
			with our scheme. Added explicit structure packing pragmas to all exposed
			structures; now we assume 68k on Mac and 2-byte on Windows.
			
			Modified DEBUG_PREFACE code so that debug-specific routines live in the library
			rather than the macros for the non-IN_X_CODE case, making it easier to swap debug
			libraries in and out.
			
			Respaced lines to allow opening in MSVC. (srj)

	950503	Release 1.1.5: Whoops! In the changes of 950405 below, interfaces with custom data
			didn't get the right amount of space allocated; thus, if you allocated
			an interface with extra storage, you would trash the heap when you
			used that data. This is all better now. This release also features
			an improved internal hashing scheme. (srj)
	950502	Release 1.1.4: add ability to not define the standard types; define
			the symbol NO_STD_GUID before including this file will cause all
			the "standard" guid's to be given external-only declarations. Added
			STD_IUNKNOWN_METHODS macro. (srj)
	950419	Release 1.1.3: added MoaBeginXtraResources and MoaEndXtraResources. (srj)
	950414	Release 1.1.2: added more standard data types. Modified
			IMoaHandle again. Revised all API calls to be of
			form STDAPI; this is important for Win32. (srj)
	950411	Release 1.1.1: modified IMoaHandle to final form. Improved
			efficiency of MoaEqualID. Correct IID_ICallback to be
			named IID_IMoaCallback. (srj)
	950405	Release 1.1: Changes to EasyShell macros to make C++ working easier.
			removed some harmless but annoying compiler warning issues. (srj)
	950404	Release 1.02: renamed IID_ICalloc, IID_IClassFactoryCallback
			to IID_IMoaCalloc and IID_IMoaClassFactoryCallback. (srj)
	950403	Release 1.01: change to prototype of DllGetClassObject to avoid compiler
			warning in MSVC2.0. Added ResEdit-style comments for the standard GUID's. (srj)
	950330	Release 1.0 for MOA. (srj)
	950323	Release 1.2. (srj)
	950321	Release 1.1. (srj)
	950308	Release 1.0b1. (srj)
*/
...
/* this must be done after all other includes */
...
/* ========================================================================== */
	...
	*/
		...
		#define INTERFACE_PREFACE									struct FAR
	...
	#define EXTERN_GUID(name) \
		EXTERN_C MOA_READONLY(MoaID) FAR name
	...
	#if defined(_MSC_VER)
		/* krd 12/12/97 fix vc 5 warnings by copying def from vc hdr rpcndr.h */
		/* another way to fix this would be to #undef EXTERN_GUID */
		#if _MSC_VER >= 1100
...
typedef IMoaUnknown FAR * PIMoaUnknown;
...
typedef IMoaClassFactory FAR * PIMoaClassFactory;
...
typedef IMoaCalloc FAR * PIMoaCalloc;
...
DECLARE_INTERFACE_(IMoaCallback, IMoaUnknown)
{
	...
	STDMETHOD_(struct IMoaCache FAR *, MoaGetCache) (THIS) PURE;

	STDMETHOD_(XtraResourceCookie, MoaBeginUsingResources) (THIS_ MoaFileRef fileRef, XtraResourceCookie FAR * pSaveCookie) PURE;
	STDMETHOD_(void, MoaEndUsingResources) (THIS_ MoaFileRef fileRef, XtraResourceCookie saveCookie) PURE;
};
typedef IMoaCallback FAR * PIMoaCallback;
...
typedef MoaClassInterfaceInfo FAR * PMoaClassInterfaceInfo;
typedef const MoaClassInterfaceInfo FAR * ConstPMoaClassInterfaceInfo;
...
#define EXTERN_XTRA_ENTRIES \
	...
	STDAPI XE_DLL(GetClassForm)(ConstPMoaClassID pClassID, MoaLong FAR * pObjSize, MoaCreatedProc FAR * pCreateProc, MoaDestroyedProc FAR * pDestroyProc); \
	STDAPI XE_DLL(GetClassInfo)(PIMoaCalloc pCalloc, ConstPMoaClassInterfaceInfo FAR * ppClassInfo); \
	...
	STDAPI DllGetClassForm(ConstPMoaClassID pClassID, MoaLong FAR * pObjSize, MoaCreatedProc FAR * pCreateProc, MoaDestroyedProc FAR * pDestroyProc); \
	STDAPI DllGetClassInfo(PIMoaCalloc pCalloc, ConstPMoaClassInterfaceInfo FAR * ppClassInfo);
...
#define _INTERFACE_STRUCTURES(macro_CLASS) \
			macro_CLASS FAR * 		pObj; 			/* pointer to the "real" object */ \
...
/* This gyration is to allow for classes/interface to be replaced by Macros.
	...
	#define MOA_CREATEI_PARMS(macro_CLASS, macro_INTF)		MoaCreate_##macro_CLASS##_##macro_INTF(macro_CLASS##_##macro_INTF FAR * This)
	#define MOA_DESTROYI_PARMS(macro_CLASS, macro_INTF)		MoaDestroy_##macro_CLASS##_##macro_INTF(macro_CLASS##_##macro_INTF FAR * This)
	
	#define MOA_CREATE(macro_NAME)					        MoaCreate_##macro_NAME
	#define MOA_DESTROY(macro_NAME)					        MoaDestroy_##macro_NAME
	
	#define MOA_CREATE_PARMS(macro_NAME)			        MoaCreate_##macro_NAME(macro_NAME FAR * This)
	#define MOA_DESTROY_PARMS(macro_NAME)			        MoaDestroy_##macro_NAME(macro_NAME FAR * This)
	...
	#define INTFQUERY_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_QI(macro_CLASS##_##macro_INTF FAR * This, ConstPMoaInterfaceID pInterfaceID, PPMoaVoid ppv)
	
	#define INTFADDREF(macro_CLASS, macro_INTF)			macro_CLASS##_##macro_INTF##_AR
	#define INTFADDREF_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_AR(macro_CLASS##_##macro_INTF FAR * This)
	
	#define INTFRELEASE(macro_CLASS, macro_INTF)		macro_CLASS##_##macro_INTF##_RL
	#define INTFRELEASE_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_RL(macro_CLASS##_##macro_INTF FAR * This)
	
	#define INTFBUILD(macro_CLASS, macro_INTF)			Build_##macro_CLASS##_##macro_INTF
	#define INTFBUILD_PARMS(macro_CLASS, macro_INTF)	Build_##macro_CLASS##_##macro_INTF(macro_CLASS FAR * pObj, PPMoaVoid ppIF)
	...
	#define CALL_BGI(macro_CLASS, macro_INTF, macro_OBJ, macro_PERR) \
													(macro_CLASS##_##macro_INTF FAR *)DLL(BuildGenInterface) \
													((MBObject FAR *)(macro_OBJ), \
	...
	#define _MOA_CREATEI_PARMS(macro_CLASS, macro_INTF)		MoaCreate_##macro_CLASS##_##macro_INTF(macro_CLASS##_##macro_INTF FAR * This)
	#define MOA_CREATEI_PARMS(macro_CLASS, macro_INTF)		_MOA_CREATEI_PARMS(macro_CLASS, macro_INTF)
	#define _MOA_DESTROYI_PARMS(macro_CLASS, macro_INTF)	MoaDestroy_##macro_CLASS##_##macro_INTF(macro_CLASS##_##macro_INTF FAR * This)
	...
	#define _MOA_CREATE_PARMS(macro_NAME)			MoaCreate_##macro_NAME(macro_NAME FAR * This)
	#define MOA_CREATE_PARMS(macro_NAME)			_MOA_CREATE_PARMS(macro_NAME)
	#define _MOA_DESTROY_PARMS(macro_NAME)			MoaDestroy_##macro_NAME(macro_NAME FAR * This)
	...
	#define _INTFQUERY_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_QI(macro_CLASS##_##macro_INTF FAR * This, ConstPMoaInterfaceID pInterfaceID, PPMoaVoid ppv)
	...
	#define _INTFADDREF_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_AR(macro_CLASS##_##macro_INTF FAR * This)
	...
	#define _INTFRELEASE_PARMS(macro_CLASS, macro_INTF)	macro_CLASS##_##macro_INTF##_RL(macro_CLASS##_##macro_INTF FAR * This)
	...
	#define _INTFBUILD_PARMS(macro_CLASS, macro_INTF)	Build_##macro_CLASS##_##macro_INTF(macro_CLASS FAR * pObj, PPMoaVoid ppIF)
	...
	#define EXTERN_BEGIN_DEFINE_CLASS_INSTANCE_VARS(macro_CLASS) \
		...
		DECLARE_INTERFACE_(macro_CLASS, IMoaUnknown) \
		{ \
			_BASE_STRUCTURES \
			BEGIN_INTERFACE \
...
// The Windows VC6 compiler demands the second delete function declaration.  
// The Mac compiler and VC5 doesn't like the second delete function. 
#if defined(WIN32) && defined(_MSC_VER) && _MSC_VER >= 1200
#define EXTRA_MOA_DELETE(x) inline void operator delete (PMoaVoid ptr, x FAR * pObj) { ptr; pObj; };
#else
#define EXTRA_MOA_DELETE(x)
	...
	#define EXTERN_BEGIN_DEFINE_CLASS_INTERFACE(macro_CLASS, macro_INTF) \
		...
		DECLARE_INTERFACE_(INTF(macro_CLASS, macro_INTF), macro_INTF) \
		{ \
			...
			STDMETHOD_(MoaUlong, AddRef) (THIS);\
			STDMETHOD_(MoaUlong, Release) (THIS); \
			inline PMoaVoid operator new (size_t size, macro_CLASS FAR * pObj) \
			{ \
				INTF(macro_CLASS, macro_INTF) FAR * p = (INTF(macro_CLASS, macro_INTF) FAR *)DLL(_MBNew)(size, pObj->pCalloc); \
				\
				if (p) { \
					p->pObj = pObj; \
					p->refCount = 1; \
			...
			INTF(macro_CLASS, macro_INTF) (MoaError FAR * pErr); \
	...
	#define EXTERN_BEGIN_DEFINE_CLASS_INSTANCE_VARS(macro_CLASS) \
		...
		EXTERN_STDMETHODIMP_(void) MOA_DESTROY_PARMS(macro_CLASS); \
		struct macro_CLASS { \
			IMoaUnknownVtbl FAR * lpVtbl; 		/* our vtable */ \
			_BASE_STRUCTURES
	...
	#define EXTERN_BEGIN_DEFINE_CLASS_INTERFACE(macro_CLASS, macro_INTF) \
		...
		EXTERN_STDMETHODIMP INTFBUILD_PARMS(macro_CLASS, macro_INTF); \
		EXTERN_C PMoaVoid DLL(BuildGenInterface)(MBObject FAR * pObj, MoaLong interfaceSize, MoaCreatedProc createdProc, PMoaVoid vtbl, MoaError FAR * pErr); \
		\
		struct INTF(macro_CLASS, macro_INTF) { \
			INTFVTBL(macro_INTF) FAR *	lpVtbl; \
			_INTERFACE_STRUCTURES(macro_CLASS) \
...
*/
	...
	*/
	#define EXTERN_GET_VTABLE(macro_CLASS, macro_INTF) \
		EXTERN_C INTFVTBL(macro_INTF) FAR * GETVTBL(macro_CLASS, macro_INTF)(void);
	...
	_MOA_PTRCHECK \
	INTFVTBL(macro_INTF) FAR THEVTBL(macro_CLASS, macro_INTF); \
	char FAR INITVTBL(macro_CLASS, macro_INTF) = 0; \
	\
	EXTERN_GET_VTABLE(macro_CLASS, macro_INTF) \
	INTFVTBL(macro_INTF) FAR * GETVTBL(macro_CLASS, macro_INTF)(void) { \
		INTFVTBL(macro_INTF) FAR * result; \
		result = &THEVTBL(macro_CLASS, macro_INTF); \
		if (!INITVTBL(macro_CLASS, macro_INTF)) { \
			INTFVTBL(macro_INTF) FAR * lpVtbl = &THEVTBL(macro_CLASS, macro_INTF); \
	...
	#define EXTERN_GET_VTABLE(macro_CLASS, macro_INTF) \
		EXTERN_C MOA_READONLY(PMoaVoid) FAR THEVTBL(macro_CLASS, macro_INTF)[sizeof(INTFVTBL(macro_INTF))/sizeof(PMoaVoid)];
...
#define STD_CLASS_CREATE_DESTROY(macro_CLASS) \
	...
	#define STD_INTERFACE_CREATE_DESTROY(macro_CLASS, macro_INTF) \
		INTF(macro_CLASS, macro_INTF)::INTF(macro_CLASS, macro_INTF)(MoaError FAR * pErr) { *pErr = (kMoaErr_NoErr); } \
	...
	#define BEGIN_DEFINE_CLASS_INTERFACE(macro_CLASS, macro_INTF) \
		...
		{ \
			...
			this->pObj->AddRef(); \
			X_RETURN(MoaUlong, ++this->refCount); \
			X_EXIT \
		} \
		 \
		...
		{ \
			...
			INTF(macro_CLASS, macro_INTF) FAR * pIF = NULL; \
	...
	#define BEGIN_DEFINE_CLASS_INTERFACE(macro_CLASS, macro_INTF) \
		...
		{ \
			...
			if (pIF) { \
				if (err) { \
					pIF->lpVtbl->Release((macro_INTF FAR *)pIF); \
					pIF = NULL; \
				} \
			} \
			else { \
	...
	#define STDINTERFACE_PREFACE \
		...
		EXTERN_C PMoaVoid DLL(BuildGenInterface)(MBObject FAR * pObj, MoaLong interfaceSize, MoaCreatedProc createdProc, PMoaVoid vtbl, MoaError FAR * pErr) \
		{ \
			MBObject_IMoaUnknown FAR * pIF = NULL; \
			PIMoaCalloc pCalloc = pObj->pCalloc; \
			\
			pIF = (MBObject_IMoaUnknown FAR *)pCalloc->lpVtbl->NRAlloc(pCalloc, interfaceSize); \
			\
			if (pIF) { \
				pIF->lpVtbl = (IMoaUnknownVtbl FAR *)vtbl; \
				pIF->pObj = pObj; \
				pIF->refCount = 1; \
...
#define _GLUE_GETCLASSFORM \
	typedef struct { \
		MoaLong FAR * pObjSize; \
		MoaCreatedProc FAR * pCreateProc; \
		MoaDestroyedProc FAR * pDestroyProc; \
		MoaBool found; \
	} _XtraGetClassFormRec; \
	STDMETHODIMP DLL(_XtraGetClassForm)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, _XtraGetClassFormRec FAR * pInfo); \
	STDMETHODIMP DLL(_XtraGetClassForm)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, _XtraGetClassFormRec FAR * pInfo) \
	{ \
	...
	STDAPI DLL(GetClassForm)(ConstPMoaClassID pClassID, MoaLong FAR * pObjSize, MoaCreatedProc FAR * pCreateProc, MoaDestroyedProc FAR * pDestroyProc); \
	STDAPIIMP DLL(GetClassForm)(ConstPMoaClassID pClassID, MoaLong FAR * pObjSize, MoaCreatedProc FAR * pCreateProc, MoaDestroyedProc FAR * pDestroyProc) { \
...
#define _GLUE_GETINTERFACE \
	...
	STDMETHODIMP DLL(_XtraGetClassInterface)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, _XtraGetClassInterfaceRec FAR * pInfo); \
	STDMETHODIMP DLL(_XtraGetClassInterface)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, _XtraGetClassInterfaceRec FAR * pInfo) \
	{ \
...
#define _GLUE_GETCLASSINFO \
	...
	STDMETHODIMP DLL(_XtraGcfInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, _XtraGetClassInfoRec FAR * pInfo); \
	STDMETHODIMP DLL(_XtraGcfInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, _XtraGetClassInfoRec FAR * pInfo) \
	{ \
	...
	STDMETHODIMP DLL(_XtraGciInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, _XtraGetClassInfoRec FAR * pInfo); \
	STDMETHODIMP DLL(_XtraGciInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, _XtraGetClassInfoRec FAR * pInfo) \
	{ \
		buildProc; \
		++pInfo->count; \
	...
	STDAPI DLL(GetClassInfo)(PIMoaCalloc pCalloc, ConstPMoaClassInterfaceInfo FAR * ppClassInfo); \
	STDAPIIMP DLL(GetClassInfo)(PIMoaCalloc pCalloc, ConstPMoaClassInterfaceInfo FAR * ppClassInfo) { \
...
#define CLASS_DEFINES_INTERFACE(macro_CLASS, macro_INTF, macro_INTERFACEVERSION) \
			...
			/*STDMETHODIMP INTFBUILD(macro_CLASS, macro_INTF)(macro_CLASS FAR * pObj, struct INTF(macro_CLASS, macro_INTF) FAR * FAR * ppIF);*/ \
