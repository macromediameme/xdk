...
/* ================================================================================
	...
	09mar98	LLB	Moved fwd decls and event structure here.
	03Apr97	jcw	Added kMuiErr_FileDialogCancelled.
	24mar97	jcw	Added #minimize to layoutStyle. Fixed purpose of file in this comment block.
	20mar97	LLB	Moved event enumeration and error constants here.
				Removed usage of push/pop from pragma pack.
	18mar97	alx Moved stuff from MuiFlags into here.
	10mar97	jcw	Minor cleanup in comments only.
	08mar97	jcw	new header file format, documentation for external consumption.
	12feb97	alx Added Non moveable window type
	05feb97	alx	Added cancel push button + changed item 150 to 160, overlap
	24jan97	JCW	Added layoutStyle, kMuiWidgetType_Void_Actor.
	21jan97	LLB	Titles and ToolTips should be ConstPMoaChar.
	14jan97	JCW	Cleanup, added a few unsupported widget types, attribs for bitmaps
	07jan97	JCW	Added flags for IMuiAlert
	17dec96	alx	Fixed TMuiWindow structure and centering enum
	11dec96	LLB	Fixed Label_Normal conflict with the Group_* enums.
	06dec96	ALL	Munge widget list. Add size to structures. Add more documentation on constants.
	05dec96	alx	Added more enums
	03dec96	JCW	added more vAttribs
	27nov96	alx	Removed modifier symbols
	26nov96	JCW	Updated information about control types.
	25nov96	alx	Added modifier symbols
	25nov96	LLB	Added pragma to push/pop struct alignment
	20nov96	***	Changed names for engineering happiness + again
				To ???? up John's elegant standards compliant naming scheme.
	19nov96	alx	Updated widget table again, added toggle button
	15nov96	JCW	Updated widget table, version implemented in, etc.
				Changed kMuiWidgetType_Button_SystemButton to kMuiWidgetType_Button_PushButton
				Removed kMuiWidgetType_Group_DefaultButtonBegin, End.
				Added kMuiWidgetType_Button_DefaultPushButton
				Changed _PopupMenu to _PopupList, _ComboBox to _EditablePopupList	
	12nov96	JCW	Changed pLimits  to vLimits, dict to MoaMmValue.
	09nov96	alx	Added items to TIMui_InputStruc (pWidgetTitle)
	06nov96	alx	Added items to TIMui_InitializationStruc
	23Oct96	JCW	Removed USING_INIT_FROM_DICT conditionals
				Munge to change MoaMui -> Mui
	21Oct96	JCW	Fixed order of structure to align nicely, (bytes at end of struc)
				Changed kIMui to kMui. Fixed up the init struc.
	17Oct96	JCW	Added toolTips to init struc. 
				Added more WidgetTypes.
	30Sep96	JCW	Reved, based on design of interactivator UI
	19Sep96	JCW	Created, based on discussions w/ lalit.
...
enum {
	...
	kMuiWidgetType_Enum_ListBox,					// 33  |		|    | not supported yet
...
typedef struct IMuiDialog FAR * PIMuiDialog;

/* forward declare MuiDlgPersist interface ptr	*/
typedef struct IMuiDlgPersist FAR *PIMuiDlgPersist;

/* forward declare MuiUrl interface pointer		*/
typedef struct IMuiUrl FAR * PIMuiUrl;

/* forward declare MuiAlert interface pointer	*/
typedef struct IMuiAlert FAR * PIMuiAlert;

/* forward declare MuiUrl interface pointer		*/
typedef struct IMuiFile FAR * PIMuiFile;
...
typedef TMuiEventRecord FAR * PTMuiEventRecord;

typedef const TMuiEventRecord FAR *ConstPTMuiEventRecord;
...
typedef const TMuiWindow FAR *ConstPTMuiWindow;
...
typedef const TMuiItem FAR *ConstPTMuiItem;
