...
File: $Workfile: moapath.h $

Abstract: Public header for IMoaPathName

History:
20aug98 greggy	Changed the bForMenu flag in GetDisplayFileName to bFullName.
07feb97 LLB 	Switch to final interfaces.
28jan97 GWR 	Added GetPathSize.
06dec96 COH 	Marked up for HTML generation
12nov96 LLB 	Added GetRelativePath.
01oct96 LLB 	Added URL style and tweaked interface.
10sep96 LLB 	Created.
*/


/* 28feb97 DDG Synched to revision 4 of this same file in Director's source
/  code (in Hopper\source\generic\moa\). */
...
typedef struct IMoaPathName FAR * PIMoaPathName;
...
The IMoaPathName interface provides various methods for the manipulation of
pathnames. 
...
*/
{
	...
	*/
	...
	STDMETHOD(SetTempPath)		(THIS_	
		PIMoaPathName pIDir)					/* on return contains an IMoaPathName pointer */	
		PURE;
	...
	*/
	...
	Creates a temporary path instance with a unique name. The client is responsible
	for releasing the returned interface when done.
	...
	*/
	...
	STDMETHOD(GetPathSize)		(THIS_
		 MoaLong FAR * pPathSize)				/* pointer to long to receive the path size */
		 PURE;
	...
	*/
	...
	STDMETHOD(GetPort)			(THIS_ 
		MoaLong FAR * pPortNumber)				/* on return contains port number */
		PURE;
	...
	*/		
	...
	STDMETHOD(GetParentDirectory)	(THIS_	
		PIMoaPathName FAR * ppIMoaPathName) 		/* on return contains a pointer to an IMoaPathName */ 
		PURE;
	...
	*/
	...
	STDMETHOD(GetRelativePath)		(THIS_	
		...
		PIMoaPathName FAR * ppRelPath)			/* on return contains a pointer to an IMoaPathName */
		PURE;
