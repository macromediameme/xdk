/*
	$Workfile: mixsnd.h $
	$Archive: /Xtras/Include/mixsnd.h $
	$Author: Greggy $
	$Date: 9/02/99 1:05p $
	$Revision: 3 $
...
}	MoaSoundFormat, FAR *PMoaSoundFormat;
...
typedef IMoaReceptorSound FAR * PIMoaReceptorSound;
...
DECLARE_INTERFACE_(IMoaReceptorSoundBuffer, IMoaReceptorSound)
{
	...
	MoaDeclareMethod(GetBuffer, (THIS_
		...
		moa_out MoaSampleBuffer FAR * sampleBuffer,
		moa_out MoaLong FAR * bufferByteSize
	))
...
typedef IMoaReceptorSoundBuffer FAR * PIMoaReceptorSoundBuffer;
...
typedef IMoaReceptorSoundStream FAR * PIMoaReceptorSoundStream;
