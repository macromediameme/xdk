...
Revision history:
12may98	greggy	Write now takes a const buffer.
15apr97	krd		Modified error codes per range assigned by Lalit
10apr97	LLB		Added MoaStream2 errors.
07feb97	LLB		Switch to final interfaces.
06dec96 COH		Marked up for HTML generation
12nov96	LLB		IOOperation is now a struct
				reduced number of new methods.
06nov96	krd		More error codes
22oct96	krd		More error codes
08oct96	krd		Added a couple of error codes
07oct96	LLB		Added GetIOAttribute and GetIOOperation.
01oct96	LLB		Added streamoperation and directbuffer methods
10sep96	LLB		First cut at MoaStream2
*/
...
*/
typedef MoaLong MoaStreamIOFlags, FAR *PMoaStreamIOFlags;


/* forward declare stream interface ptr */
typedef struct IMoaStream2 FAR *PIMoaStream2, FAR ** PPIMoaStream2;

/* forward declare stream operation ptr */
/* typedef struct MoaStreamIOOperation FAR *PMoaStreamIOOperation; */
...
typedef STDPROCPTR(STDMETHODCALLTYPE, MoaError, MoaStream2IOCompletionProc)(
	struct MoaStreamIOOperation FAR *	pStreamOp,
...

{
	...
	*/
	...
	STDMETHOD(Read)			(THIS_ 
		...
		MoaStreamCount FAR *	pNumActuallyRead)		/* number of bytes actually read */
		PURE;
	...
	*/
	...
	STDMETHOD(Write)		(THIS_ 
		...
		MoaStreamCount FAR * pNumActuallyWritten) 	/* number of bytes written */
		PURE;
	...
	*/
	...
	STDMETHOD(GetPosition)	(THIS_ 
		MoaStreamPosition FAR * pPos) /* on return contains stream position */ 
		PURE;
	...
	*/
	...
	STDMETHOD(GetModes)		(THIS_ 
		MoaLong FAR * pAccessMode,			/* on return contains access mode */ 
		MoaLong FAR * pSetPositionType) 	/* on return contains set position type */
		PURE;
	...
	*/
	...
	STDMETHOD(GetCurrentLength)		(THIS_ MoaStreamPosition FAR *pPos) PURE;
	...
	*/
	...
	STDMETHOD(GetMoaIOAttributeDict)(THIS_ 
		PIMoaIOAttributeDict FAR *ppIDict)	/* on return contains IMoaIOAttributeDict pointer */
		PURE;
